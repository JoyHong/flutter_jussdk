...
WriteRelRqst::WriteRelRqst(const String& cookie, const String& groupId)
: GroupRqst(cookie, "WriteRelRqst", groupId)
{
...
: WriteRelRqst(cookie, groupId), _added(added), _changed(changed)
{
...
bool NowaitAckChangeRelRqst::__prepare(String& err)
{
    ...
    s_pgm->_tobeFlushDatas.push_back(new UpdateGroupData(_groupId, _changed, _group->_relationUpdateTime, Status::StatusVersMap(), -1));
    if (!s_pgm->__flushData(err))
    {
        s_pgm->_tobeFlushDatas.pop_back();
        return false;
    }
...
: WriteRelRqst(cookie, ""), _relations(relations), _orgProps(orgProps)
{
...
: WriteRelRqst(cookie, ""), _orgId(orgId)
{
...
bool NowaitAckSetStatusRqst::__prepare(String& err)
{
    ...
    s_pgm->_tobeFlushDatas.push_back(new UpdateStatusesData(_groupId, statusVersMap, -1));
    if (!s_pgm->__flushData(err))
    {
        s_pgm->_tobeFlushDatas.pop_back();
        return false;
...
bool NowaitAckSetPropsRqst::__prepare(String& err)
{
    ...
    s_pgm->_tobeFlushDatas.push_back(new UpdatePropsData(_groupId, diffAndTimeStamps));
    if (!s_pgm->__flushData(err))
    {
        s_pgm->_tobeFlushDatas.pop_back();
        return false;
...
void NowaitAckSetPropsRqst::__cmdRslt(int rslt, const IputStreamPtr& iput, const ObjectPtr& userdata)
{
    ...
    if (_timeStamp > propTimeStamp.toLong(-1))
    {
        if (Group::GroupServerAgent::setProps_end(rslt, iput))
        {
            ...
            String err;
            s_pgm->_tobeFlushDatas.push_back(new UpdatePropsData(_groupId, _timeStamps));
            s_pgm->__flushData(err);
        }
...
void GetPropsRqst::__cmdRslt(int rslt, const IputStreamPtr& iput, const ObjectPtr& userdata)
{
    String err;
    StrStrMap props;
    if (Group::GroupServerAgent::getPropsPrefixs_end(rslt, iput, props))
    {
        s_pgm->_tobeFlushDatas.push_back(new UpdatePropsData(_groupId, props));
        s_pgm->__flushData(err);
    }
    else
    {
...
: Rqst(cookie, "Send" + peerUid + "MsgRqst" + ":256"), _peerUid(peerUid), _plainContent(content), _msg(Message::Msg(Message::INVALID_MSG_IDX, -1, "", content)), _inParams(inParams)
{
...
bool SendP2PMsgRqst::__prepare(String& err)
{
    ...
    if (_selfGroup->_statusVersMap.find(_peerUid) == _selfGroup->_statusVersMap.end())
    {
...
{
    map<String, BoxStatusesPtr>::const_iterator it = _selfGroup->_boxsStatuses.find(_peerUid);
...
void SendP2PMsgRqst::__fetchPeerPubKeys_begin()
{
    ...
    msgAgent.fetchPeerPubKeys_begin(this, _peerUid);
}

void SendP2PMsgRqst::__clientSend_begin()
{
    pair<map<String, BoxStatusesPtr>::iterator, bool> p = _selfGroup->_boxsStatuses.insert(make_pair(_peerUid, BoxStatuses::create(_peerUid)));
    _boxStatuses = p.first->second;
    if (p.second)
    {
        Status::StatusTimes dummyDiff;
        bool resetKeyStatuses = false;
        _boxStatuses->__set(_selfGroup->_statusVersMap[_peerUid], dummyDiff, resetKeyStatuses);
    }
    Status::StatusTimes msgStatusesDiff;
    String reason;
    if (!_boxStatuses->__prepareSendMsg(_msg, _X3DHMsgStatuses, msgStatusesDiff, _sentMsgStatusesDiff, reason))
    {
        __end(reason);
        return;
    }
    ...
    {
        s_pgm->_tobeFlushDatas.push_back(new UpdateStatusesData(_selfGroup->_groupId, _peerUid, msgStatusesDiff));
        if (!s_pgm->__flushData(reason))
        {
            s_pgm->_tobeFlushDatas.pop_back();
            __end(reason);
            return;
    ...
    msgAgent.clientSend_begin(this, "P2P/" + _peerUid, _msg._content, _inParams, callParams);
}
...
void SendP2PMsgRqst::__cmdRslt(int rslt, const IputStreamPtr& iput, const ObjectPtr& userdata)
{
    ...
    else
    {
        ...
        {
            if (_boxStatuses->__msgSent(_msg, _selfGroup->_statusVersMap[_peerUid], _sentMsgStatusesDiff))
            {
                ...
                s_pgm->_tobeFlushDatas.push_back(new InsertMsgsData(_peerUid, _msg, _sentMsgStatusesDiff));
                String err;
                s_pgm->__flushData(err);
            }
            __end("");
        ...
        else
        {
            ...
            else if ((paramIt=outParams.find(Message::K_NEED_MERGE_MSGS)) != outParams.end())
            {
                ...
                {
                    s_pgm->_tobeFlushDatas.push_back(new UpdateStatusesData(s_pgm->_selfUid, _peerUid, msgStatusesDiff));
                }
                else
                {
                    ...
                    s_pgm->_tobeFlushDatas.push_back(new InsertMsgsData(_peerUid, needMergeMsgs, msgStatusesDiff));
                }
                s_pgm->__flushData(err);
            }
            else
...
int UpdateGroupData::__flush()
{
    return s_updateGroupCb(_groupId, _diff, _relationUpdateTime, _statusVersMap, _statusTime);
}
...
int UpdateStatusesData::__flush()
{
    return s_updateStatusesCb(_groupId, _statusVersMap, _statusTime);
}
...
int UpdatePropsData::__flush()
{
    return s_updatePropsCb(_groupId, _props);
}
...
int InsertMsgsData::__flush()
{
    return s_insertMsgsCb(_groupId, _msgs, _msgStatuses);
}
...
void GroupList::__refreshMergeProps(const StrStrMap& props)
{
    ...
    s_pgm->_tobeFlushDatas.push_back(new UpdatePropsData(_groupId, diffProps));
    String err;
    s_pgm->__flushData(err);
}
...
void GroupList::__refreshMergeRels(Group::RelationsMap& roughDiff, Long relBaseUpdateTime, Long relationUpdateTime, Status::StatusVersMap& statusVersMap, Long statusQueryTime)
{
    ...
    if (relationMerged)
    {
        ...
        s_pgm->_tobeFlushDatas.push_back(new UpdateGroupData(_groupId, diff, relationUpdateTime, statusVersMap, statusQueryTime));
    }
    else if (statusQueryTime > _statusTime)
    {
        __applyStatusesDiff(statusVersMap, statusQueryTime, true);
        s_pgm->_tobeFlushDatas.push_back(new UpdateStatusesData(_groupId, statusVersMap, statusQueryTime));
    }
    String err;
    s_pgm->__flushData(err);
...
void GroupList::__notifyMergeRels(Group::RelationsMap& diff, Long relLastUpdateTime, Long relationUpdateTime, Status::StatusVersMap& addedStatusVersMap, bool fromWriteRqst)
{
        ...
        && relationUpdateTime >= _certainRelUpdateTime)
    {
        ...
        s_pgm->_tobeFlushDatas.push_back(new UpdateGroupData(_groupId, diff, relationUpdateTime, addedStatusVersMap, -1));
        String err;
        s_pgm->__flushData(err);
    }
...
void SelfGroup::__notifyMsg(const String& groupId, Message::Msg& msg)
{
    ...
    {
        s_pgm->_tobeFlushDatas.push_back(new InsertMsgsData(groupId, msg, msgStatusesDiff));
        String err;
        s_pgm->__flushData(err);
    }
    else
    {
        if (!msgStatusesDiff.empty())
        {
            s_pgm->_tobeFlushDatas.push_back(new UpdateStatusesData(_groupId, groupId, msgStatusesDiff));
            String err;
            s_pgm->__flushData(err);
        }
            ...
            if (Message::MessageDbAgent::batchRecv_end(rslt, iput, msgss))
            {
                ...
                if (_groupIdsStart.empty())
                {
                    ...
                    if (_selfGroup->__msgRecvIdle())
                    {
                        ...
                        {
                            s_pgm->_tobeFlushDatas.push_back(new UpdateStatusesData(_selfGroup->_groupId, Group::SYSTEM_GROUP_ID, keyStatusesDiff));
                            String err;
                            s_pgm->__flushData(err);
                        }
    ...
    do
    {
        ...
        else
        {
            ...
            if (msgsIt->second.empty())
                s_pgm->_tobeFlushDatas.push_back(new UpdateStatusesData(_groupId, msgsIt->first, msgStatusesDiff));
            else
                s_pgm->_tobeFlushDatas.push_back(new InsertMsgsData(msgsIt->first, msgsIt->second, msgStatusesDiff));
            s_pgm->__flushData(err);
            ++msgsIt;
        }
...
void SelfGroup::__deliverSendMsgRqsts()
{
    deque<SendP2PMsgRqstPtr>::const_iterator it = _pendingSendMsgRqsts.begin();
...
bool PGM::__flushData(String& err)
{
...
{
    s_eventProcessorCb = eventProcessorCb;
    s_loadGroupCb = loadGroupCb;
    s_updateGroupCb = updateGroupCb;
    s_updateStatusesCb = updateStatusesCb;
    s_updatePropsCb = updatePropsCb;
    s_insertMsgsCb = insertMsgsCb;
    if (getTicksCb != NULL)
        s_getTicksCb = getTicksCb;
    else
        s_getTicksCb = &getMonoTicks;
}
...
bool pgm_logined(const String& cookie, String& err)
{
    ...
    if (s_pgm)
        s_pgm->__close();
    s_pgm = new PGM();
    ERR_RET_IF(!s_pgm, "pgm_new_err");
    if (!s_pgm->__logined(cookie, err))
    {
        s_pgm = 0;
        return false;
    }
...
bool pgm_get_cur_time(Long& curTimeMs)
{
    ...
    if (!s_pgm)
        return false;
    curTimeMs = s_pgm->__getTimeStamp();
    return true;
}
...
{
    return (new NowaitAckChangeRelRqst(groupId, changedId, changedRel))->begin(err);
}

bool pgm_remove_relations(const String& cookie, const String& groupId, const StrSet& removed, String& err)
{
    return (new SetRelationsRqst(cookie, groupId, Group::RelationsMap(), Group::RelationsMap(), removed))->begin(err);
}

bool pgm_check_relation(const String& cookie, const String& peerUid, String& err)
{
    return (new CheckRelationRqst(cookie, peerUid))->begin(err);
}
...
{
    return (new ApplyRelRqst(cookie, groupId, targetId, targetType, desc, syncRelation, inParams))->begin(err);
}
...
{
    return (new AcceptRelRqst(cookie, msgIdx, targetTagName, targetTag, targetCfgs, inParams))->begin(err);
}

bool pgm_create_org(const String& cookie, const Group::RelationsMap& relations, const StrStrMap& orgProps, String& err)
{
    return (new CreateOrgRqst(cookie, relations, orgProps))->begin(err);
}

bool pgm_disband_org(const String& cookie, const String& orgId, String& err)
{
    return (new DisbandOrgRqst(cookie, orgId))->begin(err);
}
...
{
    return (new SetStatusToPeerRqst(cookie, peerUid, type, value, timeStamp))->begin(err);
}
...
bool pgm_nowait_ack_set_status(const String& groupId, const String& targetId, const String& type, const String& value, Long timeStamp, String& err)
{
    ...
    return (new NowaitAckSetStatusRqst(groupId, targetId, statusVers))->begin(err);
}

bool pgm_nowait_ack_set_props(const String& groupId, const StrStrMap& diffProps, String& err)
{
    return (new NowaitAckSetPropsRqst(groupId, diffProps))->begin(err);
}

bool pgm_get_props(const String& cookie, const String& groupId, const StrSet& prefixs, String& err)
{
    return (new GetPropsRqst(cookie, groupId, prefixs))->begin(err);
}
...
{
    return (new SendP2PMsgRqst(cookie, peerUid, content, inParams))->begin(err);
}
