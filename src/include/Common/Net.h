...
namespace Common
{
    ...
    typedef Handle<NetSender>               NetSenderPtr;
    typedef Handle<NetReceiver>             NetReceiverPtr;
    typedef Handle<StreamReceiver>          StreamReceiverPtr;
    typedef Handle<NetStream>               NetStreamPtr;
    typedef Handle<SslReceiver>             SslReceiverPtr;
    typedef Handle<NetSsl>                  NetSslPtr;
    typedef Handle<NetDriver>               NetDriverPtr;
    typedef Handle<BindChannel_Result>      BindChannel_ResultPtr;
    ...
    class CommonAPI BindChannel_Result : virtual public Shared
    {
    public:
        virtual void onBindChannel_Result(bool result,const String& reason,unsigned int channel) = 0;
        virtual void onChannelClosed(unsigned int channel) {}
    };

    /** 网络发送对象接口类 */
    class CommonAPI NetSender : virtual public Shared
    {
    public:
         ...
         * @retval      true        获取失败
         */
        virtual bool getLocal(String& host,int &port) = 0;
        /** 获取远端地址
         *
         * @param[out]  host        地址信息
         * @param[out]  port        端口信息
         ...
         * @retval      true        获取失败
         */
        virtual bool getRemote(String& host,int &port) = 0;
    };

    /** 网络接收对象接口类 */
    class CommonAPI NetReceiver : virtual public Shared
    {
    public:
        /** 收到数据回调接口
         *
    ...
    /** 可靠传输连接 接收数据处理接口 */
    class CommonAPI StreamReceiver : virtual public Shared
    {
    public:
        /** 可靠传输连接 需要输入数据
         *
         ...
         * @param[out]  data        数据内容
         */
        virtual void onIputData(Long offset,Stream& data) = 0;
        /** 可靠传输连接 输出数据
         *
         * @param[in]   offset      数据偏移量
         * @param[in]   data        数据内容
         */
        virtual void onOputData(Long offset,Stream& data) = 0;
        /** 可靠传输连接 发送数据
         *
    ...
    /** 可靠传输对象 */
    class CommonAPI NetStream : virtual public Shared
    {
    public:
        /** 关闭连接 */
        virtual void close() = 0;
         ...
         * @return      数据偏移量
         */
        virtual Long getSendOffset() = 0;
        /* receive data offset */
        /** 获取本地已经收到数据的偏移量
         *
         * @return      数据偏移量
         */
        virtual Long getRecvOffset() = 0;
    ...
    /** 安全连接数据接收接口 */
    class CommonAPI SslReceiver : virtual public Shared
    {
    public:
        /** 安全连接 输出数据
         *
    ...
    /** 安全连接对象 */
    class CommonAPI NetSsl : virtual public Shared
    {
    public:
        /** 关闭连接 */
        virtual void close() = 0;
    ...
    struct ResolveResult
    {
        ...
        bool defaultIpv6;
        vector<String> hosts4;
        vector<String> hosts6;
    };

    /** 网络接口对象 */
    class CommonAPI NetDriver : virtual public Shared
    {
    public:
        /** 创建网络接口对象
         *
         ...
         */
        virtual NetSenderPtr listen(const String& protocol,const String& localHost,int localPort,const NetReceiverPtr& receiver,bool reuseAddr = false) = 0;
        /** 连接远端地址
         *
         * @param[in]   protocol    协议类型
         * @param[in]   localHost   监听地址
         ...
         * @return      创建成功返回数据发送对象，否则返回 0
         */
        virtual NetSenderPtr connect(const String& protocol,const String& localHost,int localPort,const String& remoteHost,int remotePort,const NetReceiverPtr& receiver,bool reuseAddr = false) = 0;
         ...
         */
        virtual NetStreamPtr stream(const StreamReceiverPtr& receiver,bool realtime,Long sendOffset = 0,bool security = false) = 0;
        /** 创建安全传输连接对象
         *
         * @param[in]   receiver    数据接收处理对象
         ...
         * @return      配置值
         */
        virtual String getConfig(const String& name) = 0;
         ...
         * @return      累计发送流量，单位字节
         */
        virtual Long totalSendBytes(Long *totalBytes = 0, int *count = 0) = 0;
        /** 累计接收流量统计
         *
         * @param[in,out]   totalBytes     保存分类型统计内容的地址
         * @param[in,out]   count       分类型统计内容数量
         ...
         * @return      累计接收流量，单位字节
         */
        virtual Long totalRecvBytes(Long *totalBytes = 0, int *count = 0) = 0;
        /** 累计发送包数量统计
         *
         ...
         * @return      累计发送包数量
         */
        virtual Long totalSendPkts(Long *totalPkts = 0, int *count = 0) = 0;
        /** 累计接收包数量统计
         *
         * @param[in,out]   totalPkts     保存分类型统计内容的地址
         * @param[in,out]   count       分类型统计内容数量
         ...
         * @return      累计接收包数量
         */
        virtual Long totalRecvPkts(Long *totalPkts = 0, int *count = 0) = 0;
        /** 重置累计统计 */
         ...
         * @retval      false       解析失败
         */
        virtual bool resolveHosts(const String& domain,vector<String>& hosts4,vector<String>& hosts6,bool& defaultIpv6, int timeout = 6000) = 0;
         ...
         */
        virtual bool resolveHosts(const set<String> &domains, map<String, ResolveResult> &results, int timeout = 6000) = 0;
    };
};
