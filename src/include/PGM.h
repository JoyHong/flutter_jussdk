...
typedef int (*PGM_EVENT_PROCESSOR)(PGM_EVENT event, const StrStrMap& params);
...
*/
typedef int (*PGM_LOAD_GROUP)(const String& groupId, Group::RelationsMap& relations, Long& relationUpdateTime, Status::StatusVersMap& statusVersMap, Long& statusTime, StrStrMap& props);
...
*/
typedef int (*PGM_UPDATE_GROUP)(const String& groupId, const Group::RelationsMap& diff, Long updateTime, const Status::StatusVersMap& statusVersMap, Long statusTime);
...
*/
typedef int (*PGM_UPDATE_STATUSES)(const String& groupId, const Status::StatusVersMap& statusVersMap, Long statusTime);
...
*/
typedef int (*PGM_UPDATE_RPOPS)(const String& groupId, const StrStrMap& props);
...
*/
typedef int (*PGM_INSERT_MSGS)(const String& groupId, const Message::SortedMsgs& msgs, const Status::StatusTimes& msgStatuses);

typedef Ulong(*PGM_GET_TICKS)();
...
/*
  ...
  2. logout再login succ后需重新调用,且确保logout后,re-login前,有调用Client::close()
  3. MtcCliReconnectOkNotification回调无需调用
  4. pgm_logined内部会自动调用pgm_refresh_main, 结果通过CookieEnd事件回调
  5. 当终端本地数据未初始化
*/
...
/*
  ...
  2. 登录后或其他会导致通知丢失的场景 结束时调用. 比如后台切前台/无网切有网
*/
